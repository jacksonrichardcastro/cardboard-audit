...
export async function POST(req: Request) {
  try {
    ...
    const { kind } = body; // 'avatar' or 'banner'
    
    if (!kind || !['avatar', 'banner'].includes(kind)) {
      return new NextResponse("Valid kind ('avatar' or 'banner') is required", { status: 400 });
    }

    const filePath = `profiles/${userId}/${kind}-${Date.now()}.jpg`;
