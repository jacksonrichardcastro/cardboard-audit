...
export default async function SellerReviewPage(props: { params: Promise<{ sellerId: string }> }) {
  ...
  const userId = params.sellerId; // it's actually userId now, kept param name for routing
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      <div className="mb-6">
        <Link href="/admin" className="text-violet-400 hover:underline">&larr; Back to Master List</Link>
      </div>
      <div className="flex items-start gap-6 bg-card/50 p-8 rounded-xl border border-white/10 mb-8">
        {seller?.profilePhotoUrl ? (
          ...
          <div className="w-24 h-24 rounded-full bg-violet-600 flex items-center justify-center text-3xl font-bold">
            {(seller?.displayName || seller?.businessName || "U").charAt(0).toUpperCase()}
          </div>
        )}
        <div className="flex-1 space-y-4">
          <div>
            <h1 className="text-2xl font-bold">{seller?.displayName || seller?.businessName || "Unnamed User"}</h1>
            <p className="text-muted-foreground">{seller?.handle ? `@${seller.handle}` : "No handle"} • {dbUser.accountType}</p>
          </div>
          <div>
      ...
      {seller?.approvalStatus === "pending_review" && (
        <div className="flex flex-col gap-4 bg-card/30 p-6 rounded-xl border border-white/10">
          ...
          <form action={async (formData: FormData) => {
            ...
            await rejectSellerAction(userId, reason);
